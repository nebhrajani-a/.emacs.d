;;            /---"Exp" means Experimental...  I don't have time to work on
;;	     V	 			       this now and so I don't much...

;; hl319.el (Exp 1.33) -- customizable highlighting for Emacs19.
;; Copyright (c) 1993 Free Software Foundation, Inc.
;; Copyright (C) 1994, 1995 Jonathan Stigelman <Stig@hackvan.com>
;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; hl319.el,v 1.33 1995/02/09 14:51:55 stig Exp
;;
;; LCD Archive Entry:
;; hl319|Jonathan Stigelman|Stig@hackvan.com|
;; Comprehensive regex-based highlighting for Emacs 19|
;; 1995/02/09 14:51:55|Exp 1.33|~/packages/hl319.el.Z|
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; GENERAL OVERVIEW
...
;;
;; hl319.el,v
;; Revision 1.33  1995/02/09 14:51:55  stig
;; reversed the mapping from face names to styles...  Now,
;; hilit-lookup-face-create always returns the face that was passed to it...
;; The value that is referenced by tracing the links in the translation table
;; is the style of the face but not the name of the face.  This way, if you
;; change the user-face-table and respin the translations, all of the faces get
;; reinitialized without having to rehighlight any buffers.
;;
;; changed some of the hooks.
;;
;; (I'm still using font-lock with xemacs, but not for long...the engine gets
;; fixed next.)
;;
...
(defvar hilit-hook-list '(find-file info mail gnus dired change)
  "list of hilit19 hooks to install.  Valid values are:
    change yank yank-pop recenter find-file info mail gnus dired")
...
(defconst hilit-default-face-table
  '(
    ...
    (vm-summary-highlight-face summary-current)	; use the standard variable for this...
...
DO NOT MODIFY THIS VARIABLE.  Define hilit-user-face-table, or use hilit-translate")
...
See the documentation for hilit-translate and hilit-face-translation-table."
  ...
  (let* ((frame (selected-frame))
	 ...
	 (style face)			; preserve behavior for faces named red-bold-italic
	 fn visited tmp parent)
    ;; once upon a time, face names that had some symbolic meaning (eg:
    ;; string, comment, etc) got transmogrified into faces that had lost
    ;; their symbolic meaning in exchange for a description of their
    ;; appearance...  Now we call that appearance description a "style" and
    ;; the symbolic faces aquire the properties of the style name. 
    (catch 'done
      (if (memq style visited)
	  (error "face translation loop: %S" visited)
	(setq visited (cons style visited)
	      tmp (assq style hilit-face-translation-table))
	(if tmp
	    (setq style (cdr tmp))
	  (throw 'done nil))))
    (setq fn (symbol-name style))
    ;; now, the FACE gets the properties of the STYLE that it translated to...
    (cond
     ((null style)
      ;; do nothing if the style is nil (don't create the face)
      )
     ((or force
	  (not (memq face (face-list)))
	  (and hilit-face-check
	       (setq parent (get face 'parent-face))
	       (not (equal (face-font face) (face-font parent)))))
      ;; FIXME - this should check for resources and deal appropriately
      (make-face face)
      (if (string-match "^reverse-?" fn)
	  (progn (invert-face face)
	...
	(error "bad face name %S" face))
      ...
      (condition-case nil
	  (progn (set-face-foreground face fgcolor)
	...
	(error (message "couldn't allocate color for '%s'"
      ...
      ;; this now uses the xemacs interface...
      (let* ((bold-p (string-match "-bold" fn))
	     (italic-p (string-match "-italic" fn))
	     (parent-face (cond ((and bold-p italic-p) 'bold-italic)
				(bold-p 'bold)
				(italic-p 'italic))))
	(put face 'parent-face parent-face)
	(set-face-font face (and parent-face (face-font parent-face)) frame) ; local frame
;	(set-face-font face (and parent-face (face-font parent-face)) nil t) ; global default
    ...
    (and style (face-differs-from-default-p face) face)))
...
(defun mapcar-extents (function &optional predicate buffer from to closed-end)
  "Applies FUNCTION to all extents which overlap a region in BUFFER, starting
at FROM and ending at TO.  An optional PREDICATE may be used to further
limit the extents over which MAPFUN is mapped.  A list of the values
returned by MAPFUN is returned.  See also `map-extents'."
  (let (*result*)
		   ...
		   (function
		    (lambda (ex junk)
		      (setq *result* (cons (funcall function ex)
					   *result*))
		      nil)))
		 buffer from to nil closed-end)
    (nreverse *result*)))
;; examples:
;; (mapcar-extents 'identity (lambda (ex)
;;                             (eq 'primary-selection (extent-face ex))))
;;
;; might return a list of one extent (if you have an area highlighted)
;; (#<extent [653, 711] 0x7318b4 in buffer mapcar-extents.el>)
;;
;; and
;; (mapcar-extents 'delete-extent (lambda (ex)
;;				    (eq 'primary-selection (extent-face ex))))
;;
;; would delete the extent and return '(nil)
;;
;; GETTING A LIST OF FACES AND THEIR ENDPOINTS WOULD BE AS SIMPLE AS...
;; 
;; (apply 'nconc (mapcar-extents #'(lambda (ex) (list (extent-face ex)
...
(defun hilit-rehighlight-region (start end &optional quietly)
  ...
  (save-restriction
    ...
    (setq start (save-excursion (goto-char start)
				(beginning-of-line)
				(point))
	  start (apply 'min start (mapcar-extents
				   'extent-start-position nil nil start start))
	  end (save-excursion (goto-char end) (end-of-line) (point))
	  end (apply 'max end (mapcar-extents 'extent-end-position nil nil end end)))
    (hilit-unhighlight-region start end quietly)
    (hilit-highlight-region   start end nil quietly)))
...
(defun hilit-submit-feedback ()
  ...
  (and (y-or-n-p "Do you really want to submit a report on hl319? ")
   ...
   Y     Y   OOOO   !!!   This is Mega-Beta Code...(You might call it ALPHA.)  
	...
	Please use 'diff -cw oldver newver' for patches.
")))
...
(defun hilit-find-file-hook ()
  ...
  (cond ((and hilit-auto-highlight
	 ...
	 (and (memq 'change hilit-hook-list)
	  ...
	  (nreverse (mapcar (function
			     (lambda (x)
			       (cons (car x)
					 ...
					 (nth 1 x)))))
			    wide-table))))
  ;; reconfigure all faces that exist already...
  (let ((l (mapcar 'car hilit-face-translation-table))
	(flist (face-list)))
    (while l
      (and (memq (car l) flist)
	   (hilit-lookup-face-create (car l) t))
      (setq l (cdr l)))))
  ...
  (if (not window-system)
    ...
    (hilit-hook-install mail
      ...
      (add-hook 'vm-summary-mode-hook
		(lambda () (hilit-lookup-face-create
			    vm-summary-highlight-face t)))
    ...
    (mapcar (function
	     (lambda (ff)
	       (let ((trans (assq ff hilit-face-translation-table)))
		 (and trans (cdr trans)
		      (hilit-lookup-face-create ff)))))
	    (face-list))
    ))
...
(hilit-set-mode-patterns
 ...
 '(
   ...
   ("\\\\\\(appendix\\|tableofcontents\\|listoffigures\\|listoftables\\)\\b"
    nil decl)
   ("\\\\\\(bf\\|em\\|it\\|rm\\|sf\\|sl\\|ss\\|tt\\|sc\\)\\b" nil decl)
